...
When you add a provider, TouchAI automatically appends `/v1` to the endpoint for OpenAI-type providers and uses the base URL as-is for Anthropic-type providers.

## Built-in providers

TouchAI ships with several built-in providers pre-configured with the correct endpoints. They are disabled by default — you only need to add your API key and enable the ones you want to use.
